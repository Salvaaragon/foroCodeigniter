      ...
      <div >
        ...
        <p >
			...
			if($query->num_rows() > 0)
			{
				...
				echo '<tr class="info">
						<th class="col-sm-4">Nombre del subforo</th>
						<th class="col-sm-1"><center>Temas</th>
					  </tr>
					  </thead>';
				echo '<tbody>';
				...
				{
					echo '<tr class="active"><td class="col-sm-1" colspan="2"><h4>'
						.$row->Nombre.'</h4></td></tr>';
					$sub = $this->foro->getSubforos($row->Id);
					foreach($sub->result() as $row2)
					{
						/*echo '<tr><td class="col-sm-2"><h4>
							<a href="'.base_url().'index.php/foro/subforo/'.$row2->Id.'/true">'
							.$row2->Nombre.'</a></h4>';*/
							echo '<tr><td class="col-sm-2"><h4>
							<a href="'.base_url().'index.php/foro/subforoConLogin/'.$row2->Id.'/'.$IdUser.'/'.$nick.'">'.$row2->Nombre.'</a></h4>';

						echo '<ul><h5>'.$row2->Descripcion.'<h5></ul></td>';
						$temas = $this->foro->getNumTemas($row2->Id);
						echo '<td><center>';
						foreach($temas->result() as $row3)
							echo $row3->total.'</td></tr>';
					}
				}
